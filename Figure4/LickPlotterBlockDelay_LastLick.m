...
% %%Align Cue
function LickPlotterBlockDelay_LastLick(path, good, rewOmission, cue) 
...
%good =1 , bad = 2, All = 0
    ...
    for k = 1:size(path,1)
        ...
        LickFile = [sessname '_DataStructure_mazeSection1_TrialType1_alignLastLick_msess1.mat']; 
         load(LickFile);
        ...
        Licks = trialsLastLick.lickLfpInd;
        Start = trialsLastLick.startLfpInd;
        End = trialsLastLick.endLfpInd;
        Pump = trialsLastLick.pumpLfpInd;
     ...
     if (good == 1)
         ...
         titleStr = 'Late Lick';
     elseif  (good == 11)   
         load('LickSelectivityInd.mat')
         Trials = GoodTrLickSel
          titleStr = 'Sel Ind Good';
     elseif  (good == 2)  
     ...
     elseif (good == 0)
      ...
      % Trials = 1 %custom select trials
       ...
       if isaninteger(numFigs) == 1 
        ...
        pump = RewDel{i};
        delay =  (beh.delayLen(i)/1000000)+2;
        pumpvalue = 7.95;
        if isempty(pump)
           delay =  beh.delayLen(i)/1000000;
    ...
    ylabel('Trials', 'FontSize', 22);
     xlabel('Time from last lick (s)', 'FontSize', 22);
