function PlotClusterHeatmapsCurvesBlockOverlap(FRProfPath, KmeansPath, trType,delay)
% % PlotClusterHeatmapsCurves('Z:\Kori\immobile_code\RiseDown\tables\2025-05to05\Variable\Path4sData\FR_Prof\', 'Z:\Kori\immobile_code\RiseDown\tables\2025-05to05\Variable\Path4sData\PAC_Kmeans_noPCA_start_2\',
     ...
     cd(KmeansPath)
     load('KmeansResults3sBlock.mat')

    [~,sorted_All_ind]  = sort(catRatios3s, 'descend'); 
    data = [clusters.idx, avgFR_profile_3sDelay];
    savestr = '3s';
     elseif trType == 2
     load('Ratio5sOverlap.mat')
     cd(KmeansPath)
     load('KmeansResults5sBlock.mat')
     
    [~,sorted_All_ind]  = sort(Ratio5s, 'descend'); 
    data = [clusters.idx, avgFR_profile_5sDelay];
     savestr = '5s';
     end
