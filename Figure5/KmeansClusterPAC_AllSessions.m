function KmeansClusterPAC_AllSessions(FR_ArrayPath, savepath, doPCA, ephys, trType, timeStart,PAC, delay)
    ...
    distMetric = 'correlation'; % 'sqeuclidean' or 'correlation' (correlation groups by shape)
       ...
       elseif timeStart ==-2
        ...
        if ephys == 0
            ...
            elseif PAC ==2
              load('FRprofileTable_alignLastLickWholePop.mat')
              name = 'Population';
            elseif PAC ==3
        ...
        elseif ephys == 1
             ...
             % load('FRprofileTable_alignLastLick.mat')
             % Range = ([find(timeStepRun == -2000), find(timeStepRun == 9000)]);
             % xMax = Range(2);
              if trType == 1 
                 namestr = '';
                 % data_norm =   avgFR_profile(:,Range(1):Range(2)); 
                   elseif trType == 2
                 namestr = '3545s';
                 data_norm =   AvgFR_3545; 
                 % data_norm =   avgFR_profile_45(:,Range(1):Range(2)); 
              elseif trType == 3
                 namestr = '45s';
                 data_norm =   AvgFR_45; 
                 % data_norm =   avgFR_profile_45(:,Range(1):Range(2)); 
              ...
              elseif trType == 5 
                 namestr = '67s';
                  data_norm =   AvgFR_67; 
                 % data_norm =   avgFR_profile_67(:,Range(1):Range(2)); 
              end
        end
          cd(savepath)
        
              if doPCA == 1 && timeStart == 0
              foldername = [name '_Kmeans_PCA_start0\'];   
              elseif doPCA == 0 && timeStart == 0 
               foldername = [ name '_Kmeans_noPCA_start0_\'];   
               elseif doPCA == 1 && timeStart == -2 
               foldername = [name '_Kmeans_PCA_start_2\'];  
               elseif doPCA == 0 && timeStart == -2 
               foldername = [name '_Kmeans_noPCA_start_2\'];  
              end
         ...
         if PAC == 5
            ...
            % hold on
            % R_IntraCluster =   [R_IntraCluster1, R_IntraCluster2];
